...
friendsController.listFriends = async (req, res, next) => {
  try {
    ...
    if (req.body.user_id == null) {
      ...
      console.log(1111, "blocked_diary", userBlock);
      ...
      let users = await UserModel.find().where("_id").in(listFriendId).exec();
...
friendsController.suggest = async (req, res, next) => {
  try {
    ...
    let requested1 = await FriendModel.find({
      sender: req.userId,
      status: "1",
    }).distinct("receiver");
    let requested0 = await FriendModel.find({
      sender: req.userId,
      status: "0",
    }).distinct("receiver");
    let accepted1 = await FriendModel.find({
      receiver: req.userId,
      status: "1",
    }).distinct("sender");
    let accepted0 = await FriendModel.find({
      receiver: req.userId,
      status: "0",
    }).distinct("sender");
